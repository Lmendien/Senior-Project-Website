...
class TournamentData_TeamServiceTest extends PHPUnit_Framework_TestCase
{
	public function setUp()
	{
		$this->loadServices();
		$this->teamRecord = m::mock('Craft\TournamentData_TeamRecord');
		$this->service = new TournamentData_TeamService($this->teamRecord);
	}

	protected function loadServices()
	{
		require_once __DIR__ . '/../../services/TournamentData_TeamService.php';
	}

	public function testNewTeam()
	{
		$result = $this->service->newTeam();

		$this->assertInstanceOf('Craft\TournamentData_TeamModel', $result);
	}

	public function testNewTeamWithAttributes()
	{
		$result = $this->service->newTeam(array('id' => 5));
	...
	public function testGetAllTeams()
	{
		...
		$results = $this->service->getAllTeams();

		$this->assertEquals(2, count($results));
		$this->assertInstanceOf('Craft\TournamentData_TeamModel', $results[5]);
	}

	public function testGetTeamById()
    {
        $attributes = array('id' => 5);
        $mockRecord = m::mock('Craft\TournamentData_TeamModel');
        $this->teamRecord
            ->shouldReceive('findByPk')->with(5)
            ->andReturn($mockRecord);
        $mockRecord->shouldReceive('getAttributes')->andReturn($attributes);
        $result = $this->service->getTeamById(5);
        $this->assertInstanceOf('Craft\TournamentData_TeamModel', $result);
        $this->assertEquals(5, $result->id);
    }

    public function testGetTeamByMissingId()
    {
        $this->teamRecord->shouldReceive('findByPk')->with(5)
            ->andReturn(null);
        $result = $this->service->getTeamById(5);
    ...
    {
        $mockModel = m::mock('Craft\TournamentData_TeamModel');
        $mockModel->shouldReceive('getAttribute')->with('id')->once()->andReturn(5);
        $mockRecord = m::mock('Craft\TournamentData_TeamRecord');
        $this->teamRecord->shouldReceive('findByPk')->with(5)->once()
            ->andReturn($mockRecord);
        ...
        $result = $this->service->saveTeam($mockModel);
     ...
     */
    public function testSaveTeamNotFound()
    {
        $mockModel = m::mock('Craft\TournamentData_TeamModel');
        $mockModel->shouldReceive('getAttribute')->with('id')->once()->andReturn(5);
        $mockRecord = m::mock('Craft\TournamentData_TeamRecord');
    ...
    public function testSaveTeamInvalid()
    {
        $mockModel = m::mock('Craft\TournamentData_TeamModel');
        $mockModel->shouldReceive('getAttribute')->with('id')->once()->andReturn(5);
        $mockRecord = m::mock('Craft\TournamentData_TeamRecord');
    ...
    public function testSaveTeamNewRecord()
    {
        $mockModel = m::mock('Craft\TournamentData_TeamModel');
        $mockModel->shouldReceive('getAttribute')->with('id')->once()->andReturn(null);
        $mockRecord = m::mock('Craft\TournamentData_TeamRecord');
        $this->teamRecord->shouldReceive('create')->once()
            ->andReturn($mockRecord);
        $attributes = array('name' => 'example');
        $mockModel->shouldReceive('getAttributes')->once()->andReturn($attributes);
        $mockRecord->shouldReceive('setAttributes')->with($attributes)->once();
	...
	public function testDeleteTeamById()
    {
        ...
        $result = $this->service->deleteTeamById(5);
        $this->assertTrue($result);
    }

	public function testUndoDeleteTeamById()
	{
		$mockModel = m::mock('Craft\TournamentData_TeamModel');
        $mockModel->shouldReceive('getAttribute')->with('id')->once()->andReturn(5);
        $mockRecord = m::mock('Craft\TournamentData_TeamRecord');
        ...
        $result = $this->service->undoDeleteTeamById(5);
        $this->assertTrue($result);
	}
}